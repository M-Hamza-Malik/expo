...
<Step label="4">
  ...
  Then, in **app.json**, add [`expo.android.googleServicesFile`](/versions/latest/config/app/#googleservicesfile) with its value as the path of the **google-services.json**. If you have already generated your android folder, make sure you run `[expo prebuild]` to update it with the new app.json.
